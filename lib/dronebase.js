...
function Drone(target) {
  ...
  function send(isrequest, sequence, words) {
    ...
    if (
      ...
      throw new RangeError('invalid argument sequence');
    }

    if (typeof _words === 'string') {
      _words = _words.trim().split(' ');
    } else if (!Array.isArray(_words)) {
      throw new TypeError('invalid argument words: not an array or a string');
    }

    if (!_words.length) {
      throw new RangeError('invalid argument words: empty');
    }
    ...
    for (let i = 0; i < _words.length; ++i) {
      if (typeof _words[i] !== 'string') {
        try {
          ...
          throw new ValueError('bad word #' + (i + 1) + ': toString failed');
        }
      }

      _words[i] = _words[i].trim();
      if (!_words[i].length) {
        throw new RangeError('bad word #' + (i + 1) + ': zero chars');
      } else if (!/^[\x01-\x7F]+$/.test(_words[i])) {
        throw new ValueError('bad word #' + (i + 1) + ': invalid chars');
      }
